...
import { Database, FileJson2, FolderOpen, Globe2, Zap, Wind, HardHat, Bell, Activity } from "lucide-react";
...
const CONNECTOR_META: Record<string, { icon: typeof Database; color: string; label: string }> = {
  "equipment-sensor":   { icon: Zap,      color: "#14b8a6", label: "설비 센서" },
  "environment-sensor": { icon: Wind,     color: "#3b82f6", label: "환경 센서" },
  "worker-safety":      { icon: HardHat,  color: "#8b5cf6", label: "작업자 안전" },
  "alerts-events":      { icon: Bell,     color: "#f43f5e", label: "알림/이벤트" },
  "system-monitor":     { icon: Activity, color: "#94a3b8", label: "시스템 모니터" },
};

export interface MonitoringSourceNodeData {
  connectorId: string;
...
function MonitoringSourceNode({ data }: NodeProps<MonitoringSourceNodeData>) {
...
export default memo(MonitoringSourceNode);
