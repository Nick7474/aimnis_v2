  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex flex-col gap-4 lg:gap-6 flex-1 overflow-auto pb-[20px] pr-2 custom-scrollbar">
          ...
          <div className="flex flex-col gap-4 lg:gap-6 flex-1 min-h-0">
             ...
             <div className="flex flex-wrap items-end gap-4 shrink-0">
               <div className="flex flex-col w-[200px]">
                 ...
                 <div className="relative">
                   ...
                   <input type="text" placeholder="사용자명 입력" className="rounded-lg pl-8 pr-3 py-1.5 text-xs text-slate-200 outline-none w-full border" style={{ background: surface, borderColor: border }} />
                 </div>
               </div>

               <div className="flex flex-col">
                 <span className="text-[10px] text-slate-500 mb-1 ml-1">소속</span>
                 <select className="rounded-lg px-3 py-1.5 text-xs text-slate-300 outline-none w-[120px] border" style={{ background: surface, borderColor: border }}>
                   <option>전체</option>
                 </select>
               </div>
               ...
               <div className="ml-auto flex items-center gap-2">
                 ...
                 <button className="flex items-center gap-1.5 hover:bg-[#334155] border border-[#334155] text-slate-300 text-xs px-4 py-1.5 rounded-lg transition-colors" style={{ background: bg }}>
                   <Lock size={14} /> 권한 저장
                 </button>
               </div>
             </div>
             ...
             <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 shrink-0 lg:h-[400px]">
               ...
               <div className="rounded-xl flex flex-col w-full lg:w-[60%] overflow-hidden h-full border" style={{ background: surface, borderColor: border }}>
                 <div className="px-4 py-3 border-b flex items-center gap-2 text-sm font-bold text-slate-300" style={{ background: surface, borderColor: border }}>
                    사용자 목록
                 </div>
                 <div className="overflow-x-auto flex-1 custom-scrollbar" style={{ background: `${bg}80` }}>
                    <table className="w-full text-left whitespace-nowrap text-xs">
                      <thead className="text-slate-400 border-b" style={{ borderColor: border }}>
                        <tr>
                      ...
                      <tbody className="divide-y divide-[#1f2937]">
                        {mockUsers.map((user, i) => (
                           <tr key={user.id} className="transition-colors cursor-pointer" style={i === 0 ? { background: `${surface}` } : {}}>
               ...
               <div className="rounded-xl flex flex-col w-full lg:w-[40%] overflow-hidden h-full border" style={{ background: surface, borderColor: border }}>
                 ...
                 <div className="p-4 flex flex-col gap-5 overflow-y-auto custom-scrollbar flex-1" style={{ background: bg }}>
                    ...
                    <div className="grid grid-cols-5 gap-y-3 gap-x-2 text-xs items-center">
                       ...
                       <div className="col-span-4"><input type="text" value={mockUsers[0].name} readOnly className="w-full border rounded p-1.5 text-slate-300 outline-none" style={{ background: surface, borderColor: border }} /></div>

                       <div className="col-span-1 text-slate-400">소속</div>
                       <div className="col-span-4">
                         <select className="w-full border rounded p-1.5 text-slate-300 outline-none" style={{ background: surface, borderColor: border }}>
                       ...
                       <div className="col-span-4"><input type="text" value="010-1234-5678" readOnly className="w-full border rounded p-1.5 text-slate-400 outline-none" style={{ background: surface, borderColor: border }} /></div>
                    ...
                    <div className="mt-2 text-xs">
                       <div className="text-slate-400 font-bold mb-3 border-b pb-1" style={{ borderColor: border }}>접근 가능 메뉴</div>
        ...
        {activeTab === '설비/구역 관리' && (
           <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 flex-1 min-h-0">
             <div className="rounded-xl flex flex-col w-full lg:w-[30%] overflow-hidden border" style={{ background: surface, borderColor: border }}>
               ...
               <div className="p-4 flex-1 overflow-y-auto text-sm text-slate-300 custom-scrollbar" style={{ background: bg }}>
             ...
             <div className="rounded-xl flex flex-col w-full lg:w-[70%] overflow-hidden border" style={{ background: surface, borderColor: border }}>
                ...
                <div className="p-6 flex-1 flex flex-col gap-6" style={{ background: bg }}>
                   <div className="grid grid-cols-4 gap-4 items-center text-sm">
                      ...
                      <div className="col-span-3"><input type="text" value="EQ-A-001" className="w-full border rounded p-2 text-slate-300 outline-none" style={{ background: surface, borderColor: border }} readOnly/></div>

                      <div className="text-slate-400">설비명</div>
                      <div className="col-span-3"><input type="text" defaultValue="압축기 #A" className="w-full border rounded p-2 text-slate-300 outline-none" style={{ background: surface, borderColor: border }} /></div>

                      <div className="text-slate-400">소속 구역</div>
                      <div className="col-span-3">
                        <select className="w-full border rounded p-2 text-slate-300 outline-none" style={{ background: surface, borderColor: border }}>
                      ...
                      <div className="col-span-3 flex items-center gap-2">
                         <input type="number" defaultValue="30" className="w-24 border rounded p-2 text-slate-300 outline-none" style={{ background: surface, borderColor: border }} /> 일
                      </div>
                   </div>
                        ...
                        { type: '심박동 (작업자)', unit: 'bpm', normal: '< 110', warn: '110', danger: '130' }
                         ...
                         <tr key={s.type} className="hover:bg-[#1e293b]/30">
                            ...
                            <td className="py-3 text-center">
                               <input type="number" defaultValue={s.warn} className="w-16 border border-yellow-500/30 text-center rounded p-1 text-slate-300 outline-none focus:border-yellow-500" style={{ background: surface }} />
                            </td>
                            <td className="py-3 text-center">
                               <input type="number" defaultValue={s.danger} className="w-16 border border-red-500/30 text-center rounded p-1 text-slate-300 outline-none focus:border-red-500" style={{ background: surface }} />
                            </td>
                         </tr>
                      ))}
                   </tbody>
        ...
        {activeTab === '데이터 연동' && (
           <div className="rounded-xl flex flex-col h-full overflow-hidden border" style={{ background: surface, borderColor: border }}>
             <div className="px-4 py-3 border-b flex justify-between items-center text-sm font-bold text-slate-300" style={{ background: surface, borderColor: border }}>
                외부 시스템 및 디바이스 연동
                <button className="hidden text-xs py-1 px-3 border border-[#334155] rounded-md text-slate-300 hover:bg-[#334155] md:block transition-colors" style={{ background: bg }}>새로운 연동 추가</button>
             </div>
             ...
             <div className="p-6 flex-1" style={{ background: bg }}>
                <div className="max-w-2xl flex flex-col gap-8">
                   ...
                   <div className="flex flex-col gap-2">
                     ...
                     <div className="flex items-center gap-4">
                        <button className="flex-1 p-4 border-2 rounded-lg text-slate-200 flex flex-col items-center gap-2" style={{ background: bg, borderColor: primary }}>
                   ...
                   <div className="grid grid-cols-2 gap-8">
                      <div className="flex flex-col gap-2">
                         ...
                         <select className="border border-[#334155] rounded-lg p-2 text-slate-300 outline-none w-full" style={{ background: surface }}>
                           <option>10초</option>
