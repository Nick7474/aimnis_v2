      ...
      <div className="flex flex-col gap-4 lg:gap-6 flex-1 overflow-auto pb-[20px] pr-2 custom-scrollbar">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 shrink-0">
          ...
          <div className="rounded-xl px-5 py-4 flex gap-4 items-center h-[100px] shrink-0 border" style={{ background: surface, borderColor: border }}>
            ...
            <div className="flex flex-col flex-1">
               <div className="text-slate-400 text-sm font-medium">위험/경고 건수</div>
          ...
          <div className="rounded-xl px-5 py-4 flex gap-4 items-center h-[100px] shrink-0 border" style={{ background: surface, borderColor: border }}>
            ...
            <div className="flex flex-col flex-1">
               <div className="text-slate-400 text-sm font-medium">조치 완료율</div>
          ...
          <div className="rounded-xl px-5 py-4 flex gap-4 items-center h-[100px] shrink-0 border" style={{ background: surface, borderColor: border }}>
            ...
            <div className="flex flex-col flex-1">
               <div className="text-slate-400 text-sm font-medium">반복 발생 대상</div>
               <div className="text-3xl font-bold text-slate-100 mt-0.5">7 <span className="text-sm font-normal text-slate-500">건</span></div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 shrink-0 border p-3 rounded-xl" style={{ background: surface, borderColor: border }}>
           <div className="flex flex-col">
             ...
             <div className="flex items-center rounded-lg p-0.5 h-[32px]" style={{ background: bg }}>
               ...
               <button className="px-3 py-1 text-xs rounded text-slate-400 hover:text-slate-200 flex items-center gap-1 border-l border-[#334155] ml-1 pl-3">
              ...
              <div className="flex flex-1 items-center">
                 <div className="w-[140px] h-[140px] shrink-0 relative">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        ...
                        <Tooltip contentStyle={{ backgroundColor: '#1e293b', borderColor: '#334155', fontSize: '11px' }} itemStyle={{ color: '#fff' }} />
                      </PieChart>
                    </ResponsiveContainer>
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                       <span className="text-[10px] text-slate-500 mb-0.5">총 86건</span>
                       <span className="text-lg font-bold text-slate-200">15</span>
                    </div>
                 </div>
                 <div className="flex-1 ml-4 text-[11px]">
                    ...
                    {mockPieData.map(item => (
                      <div key={item.name} className="flex items-center justify-between py-1.5 text-slate-300">
                 ...
                 <table className="w-full text-left whitespace-nowrap text-xs">
                    ...
                    <tbody className="divide-y divide-[#1f2937]/50 text-slate-300">
                      {mockIssues.filter(i => activeTab === '일일 리포트' || activeTab === '조치 이력 리포트' || i.type === activeTab.replace(' 리포트', '')).map(issue => (
                        <tr key={issue.id} className="hover:bg-[#1e293b]/30">
                           <td className="px-4 py-2.5 text-center text-slate-400">{issue.id}</td>
                           ...
                           <td className="px-3 py-2.5 text-slate-400">{issue.location}</td>
                           <td className="px-3 py-2.5 text-center font-bold text-slate-200">{issue.count}</td>
                           <td className="px-3 py-2.5 text-center">
                           ...
                           <td className="px-3 py-2.5 text-center text-slate-400">{issue.time}</td>
                           <td className="px-3 py-2.5 text-center text-slate-500"><ChevronRight size={14} className="mx-auto" /></td>
                        </tr>
                      ))}
                    </tbody>
                 </table>
                    ...
                    <tbody className="divide-y divide-[#1f2937]/50 text-slate-300">
                      {mockHistory.map(item => (
                        <tr key={item.id} className="hover:bg-[#1e293b]/30">
                           ...
                           <td className="px-4 py-2.5 text-slate-400">{item.period}</td>
                           <td className="px-4 py-2.5 text-slate-400">{item.time}</td>
                           <td className="px-3 py-2.5 text-center">
                              {item.status === '완료' ? (
