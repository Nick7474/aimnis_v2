    ...
    <div className="flex flex-col w-full h-full">
      <div className="px-4 py-3 lg:px-5 lg:py-4 border-b border-[#1f2937]/50 shrink-0">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-bold text-white tracking-wide">설비 이상 현황</h2>
      ...
      <div className="flex flex-col flex-1 p-4 lg:p-5 gap-4 overflow-y-auto custom-scrollbar">
        ...
        <div className="text-xs text-slate-500 flex justify-between w-full mt-2 lg:hidden">
            <span>이상 건수</span>
            <span>이상 건수</span>
        </div>
      </div>
      ...
      <div className="w-full h-48 shrink-0 relative">
         <div className="absolute top-0 text-[10px] text-slate-500 left-0 hidden lg:block">이상 건수<br/>100</div>
         <div className="absolute top-0 text-[10px] text-slate-500 right-0 hidden lg:block">이상 건수<br/>100</div>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 20, right: 20, left: 20, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#1f2937" vertical={false} />
            <XAxis dataKey="time" stroke="#475569" fontSize={11} tickLine={false} axisLine={false} tickMargin={10} minTickGap={20} />
      ...
      <div className="flex flex-col gap-2 shrink-0">
        <div className="flex justify-between items-end shrink-0">
            <h3 className="text-sm font-bold text-slate-200">설비 목록 <span className="text-[10px] font-normal text-slate-500">(단위: 대, 건수)</span></h3>
          ...
          <table className="w-full text-xs text-left">
            ...
            <tbody className="divide-y divide-[#1f2937]/50">
              {tableData.slice(0, 4).map((row) => (
                <tr key={row.id} className="text-slate-300 hover:bg-[#1f2937]/30 transition-colors">
                  <td className="py-2 px-2 whitespace-nowrap text-slate-400">{row.id}</td>
                  <td className="py-2 px-2 whitespace-nowrap">{row.name}</td>
                  <td className="py-2 px-2 whitespace-nowrap text-slate-400">{row.location}</td>
                  ...
                  <td className="py-2 px-2 truncate hidden sm:table-cell text-slate-400 max-w-[120px]" title={row.issue}>{row.issue}</td>
                  <td className="py-2 px-2 whitespace-nowrap text-right text-slate-400">{row.time}</td>
                </tr>
              ))}
            </tbody>
          </table>
