...
export async function POST(req: NextRequest) {
  const { prompt, provider = "gemma4" } = await req.json();
  const userText = (prompt as string) ?? "";

  // Claude 경로
  if (provider === "claude-sonnet" || provider === "claude-haiku") {
    try {
      const model = provider === "claude-sonnet" ? SONNET_MODEL : HAIKU_MODEL;
      const stream = await claudeStream(model, userText);
    ...
    async start(controller) {
      ...
      try {
        const res = await fetch(`${OLLAMA_URL}/api/generate`, {
          ...
          body: JSON.stringify({ model: OLLAMA_MODEL, system: SYSTEM_PROMPT, prompt: userText, stream: true }),
        });
        ...
        controller.enqueue(enc.encode(mockResponse(userText)));
      }
      controller.close();
    },
  });
