# AIMNIS POC 구현·검수 정의서 — 개발 구현 계약서 v1.2

> 대상: 백엔드/풀스택 개발자 · QA
> 목적: 이 문서 하나로 **1차 POC 구현 범위**와 **완료(검수) 기준**을 **계약 수준으로 확정**한다.
> 작성 근거: 현재 리포지토리의 실제 코드·라우팅·상태관리·목업 데이터 직접 분석 (화면 추측 아님)
> 작성일: 2026-07-15 · 문서 등급: **구현 계약서 v1.2(확정)** · 기준 브랜치: `main`
> 표기 원칙: **[현재 코드]** = 리포에서 실측된 사실 / **[POC 목표]** = 이번 계약으로 확정된 구현 목표. 둘을 혼동하지 말 것.

> **변경 이력**
> - **v1.2** — ① 위젯 생성 구조 구체화(공통 Widget Schema + 유형별 Config Schema 분리) ② 스타일 프리셋 5~6종 적용 ③ AI 서버와 Web·DB 서버의 **2대 서버 구성 확정** ④ Mock JSON 검증 후 Gemma 4 연결하는 **구축 순서** 추가 ⑤ 용어 정리(`WidgetInstance`/`WidgetTemplate`)
> - **v1.1** — Widget Schema Engine 방향 추가(AI는 코드가 아닌 Widget Schema 생성)
> - **v1.0** — 코드 직접 분석 기반 POC 범위·검수 기준 계약 확정
...
- [ ] **DoD-11** **Widget Schema Engine** (v1.1 신규 · [0.6](#06-widget-schema-engine-아키텍처-v11-신규), [7-A](#7-a-widget-schema-engine-조합형-위젯-아키텍처--v11-신규))
  ...
  - 생성된 Schema는 **Spring Boot Backend Validation을 통과**해야 한다(스키마 무결성 + Type/Config/Style/Field 화이트리스트 검증).
  - **Preview 가능**해야 한다(저장 전 미리보기 렌더).
  - **Save 가능**해야 한다(서버 DB에 `WidgetInstance` 저장, 재사용 시 `WidgetTemplate`).
  - **재사용 가능**해야 한다(저장된 `WidgetTemplate`을 다른 페이지/프로젝트에서 재배치).
...
### 0.5 POC 인프라 구성 — 2대 물리 서버 (v1.2 확정)
> **최신 확정:** 1차 POC의 물리 서버는 **총 2대**다. (v1.1의 "한 장비에서 논리적으로 분리" 표현은 폐기하고 아래 2대 구성이 우선한다.) 아래 3역할(AI 추론 / 백엔드·검증 / DB·데이터)은 **2대 서버에 물리적으로 배치**된다.

**서버 1 — Gemma 4 AI 서버**
- **역할**: Gemma 4 모델 실행 · 내부 추론 API 제공 · **구조화된 Widget Action 또는 Widget Schema JSON 반환** · GPU/모델 상태 및 추론 로그 관리 · Health Check
- **설치·실행**: Ubuntu LTS · NVIDIA GPU Driver · Docker Engine · NVIDIA Container Toolkit · vLLM · Gemma 4 후보 모델
- **설치하지 않음**: Spring Boot · PostgreSQL · Next.js · Nginx
- **접근 통제**: AI 서버의 추론 API는 **Web 서버(서버 2)에서만** 접근 가능.

**서버 2 — AIMNIS Web·DB 서버**
- **역할**: 프론트 서비스 · Spring Boot API · Gemma 4 호출 · **Widget Schema 및 Tool Call 검증** · 데이터 조회·바인딩 · 위젯·프로젝트·로그 저장 · PostgreSQL 운영
- **Docker 컨테이너**: `nginx` · `aimnis-frontend` · `aimnis-backend` · `postgres`
- **기본 기술**: Ubuntu LTS · Docker Engine · Docker Compose · Nginx · Next.js · Java 21 · Spring Boot · PostgreSQL · Flyway

**불변 원칙 (굵게)**
- **Spring Boot는 Web·DB 서버(서버 2)에서만 실행한다.**
- **AI 서버(서버 1)에는 Spring Boot를 설치하지 않는다.**
- **프론트엔드는 Gemma 4 서버와 PostgreSQL에 직접 접근하지 않는다.**
- **모든 요청·검증·저장·실행은 Spring Boot를 거친다.**

**전체 흐름**
```text
사용자 브라우저 → Nginx / Next.js → Spring Boot → PostgreSQL 또는 AI 서버의 Gemma 4
```

> 장비의 정확한 CPU·GPU·메모리·저장장치와 Gemma 4 세부 모델은 견적·벤치마크 전까지 **`후보(검증 전)`** 상태를 유지한다([0.2](#02-장비모델-표기-규칙-문서-전체-통일)). 하드웨어 숫자를 임의로 확정하지 말 것.

### 0.5.1 POC 구축 순서 (v1.2 신규)
> **핵심 원칙: `Gemma 4부터 연결하지 않는다.`** 먼저 고정된 Mock JSON으로 `사용자 명령 → Spring Boot 검증 → WidgetInstance 저장 → 프론트 표시 → 새로고침·재접속 복원` 흐름을 완성한 뒤, **Mock AI Provider를 Gemma 4 Provider로 교체**한다.

1. 서버 공통 준비(Ubuntu·Docker)
2. AI 서버 **GPU·Docker 인식 확인**(NVIDIA Driver + Container Toolkit)
3. PostgreSQL 구축
4. Spring Boot 기본 API + DB 저장·조회 구현
5. **Gemma 4 없이 Mock JSON으로 위젯 1개 생성·저장·복원 검증**([9.0](#90-최초-위젯-생성-검수-기준-v12--dod-11-선행))
6. vLLM + Gemma 4 실행
7. Spring Boot ↔ Gemma 4 연결
8. 기존 Next.js 프론트 연결
9. Spec Board 기반 위젯 **4~6개** 생성([4.4](#44-현장-요구spec--gemma-4--실제-위젯-구성-반영-item-4))
10. **폐쇄망 전체 E2E 3회 연속** 검수([9.1](#91-반복-안정성-기준-dod-10--item-6))
...
> **v1.2 위젯 생성 정의:** 새 위젯 생성 = 아래 4요소를 조합해 **새로운 `WidgetInstance`** 를 만드는 것이다(자유 React/HTML/JS 코드 생성 아님).
> ```text
> 공통 Widget Schema + 10종 유형별 Config Schema + Style Preset + Data Binding = WidgetInstance
> ```
> - 확정된 **10종 Widget Type** 중 하나 · 해당 Type의 **유형별 Config Schema** · **5~6종 Style Preset** 중 하나 · 등록된 내부 데이터 소스와 **Data Binding**.
> - **모든 위젯을 하나의 거대한 스키마로 처리하지 않는다.** 공통 스키마는 얇게 유지하고, `config`는 Type별 스키마로 **별도 검증**한다([7-A.3](#7-a3-widget-schema-구조-계약)).
                        ...
                        Widget Schema Generator       ← Widget Schema JSON 생성(코드 아님)
                          ...
                          Data Binding Engine           ← Schema binding ↔ DataSource 필드 연결(백엔드 경유)
                               ...
                               Preview                  ← 저장 전 미리보기 렌더(격리)
                                     │
                                     ▼
                                Save                    ← WidgetInstance 저장 / (선택) WidgetTemplate로 재사용
```

- **각 단계는 Spring Boot 백엔드(서버 2)가 오케스트레이션**한다. Gemma 4는 Generator 단계의 산출물(Schema)만 담당하고, **Spring Boot 검증을 통과하지 못한 Schema는 폐기**(Preview·저장·화면 반영 불가)한다.
...
update_widget_style        # (v1.2) Style Preset 변경 — Widget Type 유지
change_widget_type         # (v1.2) Widget Type 변경 — Config·Data Binding 재검증
move_widget                # 위치 변경
resize_widget              # 크기 변경
```
- **(v1.2) 변경 의미 구분** — 백엔드는 다음을 다르게 처리한다:
  - “더 작고 단순하게 보여줘” → **Widget Type 유지 + Style Preset만 변경**(`update_widget_style`). Config·Data Binding 재검증 불필요.
  - “선 그래프 대신 현재값을 크게 보여줘” → **Widget Type 변경**(`change_widget_type`) → **Config Schema와 Data Binding을 다시 검증**.
...
validate_widget_schema     # 생성된 Schema를 Spring Boot 규칙/화이트리스트로 검증
preview_widget             # 검증된 Schema를 격리 렌더로 미리보기
save_custom_widget         # 검증·미리보기 통과한 위젯을 WidgetTemplate로 저장(재사용)
reuse_custom_widget        # 저장된 WidgetTemplate을 다른 페이지/프로젝트에 재배치
```
- Tool 인자의 `widgetType`·`stylePreset`·`dataSource`·`field`·`config`는 **10종 Type · Style Preset · 데모 커넥터 필드 · 유형별 Config Schema 화이트리스트**로 제한([7-A](#7-a-widget-schema-engine-조합형-위젯-아키텍처--v11-신규)).
- Gemma 4 결과는 **반드시 Spring Boot 백엔드(서버 2)의 검증을 통과한 경우에만** Preview·저장·화면 반영이 가능하다. `validate_widget_schema` 실패 시 **폐기**.
...
| `WidgetInstance` | **대시보드에 실제 생성된 위젯(v1.2)** — 공통 Schema + config + stylePreset + dataBinding([7-A.3](#7-a3-widget-schema-구조-계약)) | 스냅샷 `widgets.items`(localStorage) |
| `WidgetTemplate` | **재사용용 저장 위젯 설정(v1.2)** — 검증 통과한 WidgetInstance를 템플릿화 | (현재 없음, v1.2 신규) |
...
> **`WidgetInstance` / `WidgetTemplate`(v1.2, v1.1 `CustomWidget` 대체)**: `generate→validate→preview→save`([7-A](#7-a-widget-schema-engine-조합형-위젯-아키텍처--v11-신규))를 통과한 위젯의 정본. `WidgetInstance`는 한 화면의 실제 위젯이고, `WidgetTemplate`은 재사용을 위해 저장한 설정이다. 최소 필드: `id` · `widgetType` · `title` · `config` · `stylePreset` · `dataBinding` · `schemaVersion` · `createdBy` · `reusableScope`(page|project|tenant) · `createdAt` · `updatedAt`. `reuse_custom_widget`은 `WidgetTemplate`을 다른 페이지/프로젝트에 재배치한다.
...
<!-- v1.2 구체화: 공통 Widget Schema + 유형별 Config Schema + Style Preset 분리 -->
...
>
> **불변 원칙:** ① AI는 **코드가 아니라 Widget Schema(JSON)** 를 만든다. ② Schema는 **등록된 것만** 참조한다(Type/Config/Style/Field 화이트리스트). ③ **Spring Boot 검증 통과분만** 렌더·저장된다. ④ 자유 코드 경로(React/JS/HTML)는 **1차 POC에 존재하지 않는다**([10.1](#101-2차-poc-범위-v11-명시)).

> **v1.2 위젯 생성 정의 & 용어**
> ```text
> 공통 Widget Schema + 10종 유형별 Config Schema + Style Preset + Data Binding = WidgetInstance
> ```
> - **`WidgetInstance`**: 대시보드에 실제 생성된 위젯(한 화면의 한 위젯).
> - **`WidgetTemplate`**: 검증 후 저장해 다른 페이지/프로젝트에서 재사용하는 위젯 설정.
> - **자유로운 신규 React 컴포넌트 생성은 1차 POC 제외**([10.1](#101-2차-poc-범위-v11-명시)). (v1.1의 `CustomWidget`은 이 두 용어로 대체한다.)
...
**(v1.2 구체화 — 공통 Widget Schema + 유형별 Config Schema 분리)**

> **모든 위젯을 하나의 거대한 스키마로 처리하지 않는다.** **공통 Widget Schema는 얇게** 유지하고, 위젯 유형별 세부는 `config`에 담아 **Widget Type별 Config Schema로 별도 검증**한다.

**공통 Widget Schema (모든 위젯 공통 · 아래 항목만 포함)**

| 키 | 타입 | 설명 |
|----|------|------|
| `widgetId` | string | 위젯 인스턴스 식별자 |
| `widgetType` | enum | 10종 Widget Type 중 하나([7.1](#71-poc-표준-widget-type-10종--gemma-4-제작대상-item-2)) |
| `title` | string | 위젯 제목 |
| `description` | string | 설명 |
| `layout` | object | 그리드 배치(`{ x, y, w, h }`) |
| `stylePreset` | enum | Style Preset 하나([7-A.3.2](#7-a32-style-preset-5-6종--v12)) |
| `dataBinding` | object | 슬롯 ↔ DataSource 필드 매핑(`dataSourceId`+`field`, 백엔드 경유) |
| `config` | object | **Widget Type별 스키마로 별도 검증**([7-A.3.1](#7-a31-유형별-config-schema-10종)) |
| `schemaVersion` | string | 스키마 버전 |
| `createdBy` | string | 생성 주체(user/ai) |
| `version` | number | 위젯 인스턴스 리비전 |

#### 7-A.3.1 유형별 Config Schema (10종)
`config`는 `widgetType`에 대응하는 스키마로만 검증한다(다른 Type의 필드가 오면 거부). 유형별 주요 항목은 [7.2](#72-10종-편집-기능-계약--공통-필수--조건부-item-3)의 조건부 편집 기능과 1:1 대응한다.

| Widget Type | `config` 주요 키(예) |
|-------------|----------------------|
| KPI | `valueField`, `aggregation`, `unit`, `threshold` |
| 상태 요약 | `stateField`, `stateMapping` |
| 라인 차트 | `timeField`, `valueField`, `seriesField`, `unit` |
| 막대 차트 | `groupField`, `valueField`, `aggregation`, `sort` |
| 도넛 차트 | `groupField`, `valueField`, `aggregation` |
| 데이터 테이블 | `columns[]`, `sort`, `pageSize` |
| 게이지 | `valueField`, `min`, `max`, `threshold`, `unit` |
| 히트맵 | `xField`, `yField`, `valueField`, `max` |
| 알림·이벤트 목록 | `displayFields`, `severityField`, `sort`, `limit` |
| 설비 상태·헬스 카드 | `equipmentField`, `statusField`, `threshold` |

#### 7-A.3.2 Style Preset (5~6종 · v1.2)
표시 스타일은 **아래 6종 Style Preset** 중 하나로 지정한다(자유 CSS/코드 아님). 스타일 변경은 Widget Type·Config·Data Binding을 바꾸지 않는다.

| Style Preset | 의미 |
|--------------|------|
| `STANDARD` | 기본 표준 표시 |
| `COMPACT` | 축약·고밀도 표시 |
| `HIGHLIGHT` | 핵심 값 강조(크게) |
| `STATUS` | 상태색 중심 표시 |
| `COMPARISON` | 비교(목표 대비 등) 표시 |
| `MINIMAL` | 라벨·장식 최소 |

> **모든 Widget Type이 모든 Style을 지원하지는 않는다.** 각 Type에 **허용 Style 목록**을 정의하고, **허용되지 않은 조합은 백엔드가 거부**한다.

**Widget Type ↔ 허용 Style 매트릭스(기본 정의, 벤치마크·디자인 확정 시 조정)**

| Widget Type | 허용 Style |
|-------------|-----------|
| KPI | STANDARD, COMPACT, HIGHLIGHT, COMPARISON, MINIMAL |
| 상태 요약 | STANDARD, COMPACT, STATUS, MINIMAL |
| 라인 차트 | STANDARD, COMPACT, COMPARISON, MINIMAL |
| 막대 차트 | STANDARD, COMPACT, COMPARISON |
| 도넛 차트 | STANDARD, COMPACT, MINIMAL |
| 데이터 테이블 | STANDARD, COMPACT |
| 게이지 | STANDARD, HIGHLIGHT, STATUS, MINIMAL |
| 히트맵 | STANDARD, COMPACT |
| 알림·이벤트 목록 | STANDARD, COMPACT, STATUS |
| 설비 상태·헬스 카드 | STANDARD, STATUS, HIGHLIGHT |

> 디자인 표현은 **`AIMNIS Design System_v2.0.html`** 디자인 시스템을 따른다([2](#2-기술-스택--실행-전제) 디자인 규칙). Style Preset은 그 토큰 위에서의 표시 변형이다.
...
### 7-A.7 WidgetInstance 예제 (JSON · v1.2)
"전체 설비 상태" KPI 위젯을 **공통 Schema + Config + Style Preset + Data Binding** 으로 조합한 `WidgetInstance` 예시:
```json
{
  "widgetId": "w-eq-status-001",
  "widgetType": "KPI",
  "title": "전체 설비 상태",
  "description": "정상 설비 수 KPI",
  "layout": { "x": 0, "y": 0, "w": 3, "h": 2 },
  "stylePreset": "HIGHLIGHT",
  "dataBinding": {
    "valueField": { "dataSourceId": "equipment-status", "field": "normalCount" }
  },
  "config": {
    "valueField": "normalCount",
    "aggregation": "count",
    "unit": "대",
    "threshold": { "warn": 90, "danger": 80 }
  },
  "rules": [
    { "op": "threshold", "field": "normalCount", "lt": 80, "then": "danger" }
  ],
  "schemaVersion": "widget.v1",
  "createdBy": "ai",
  "version": 1
}
```
- `widgetType`은 [7.1](#71-poc-표준-widget-type-10종--gemma-4-제작대상-item-2) 10종 화이트리스트, `config`는 [7-A.3.1](#7-a31-유형별-config-schema-10종) KPI 스키마로만 검증.
- `stylePreset`은 [7-A.3.2](#7-a32-style-preset-5-6종--v12) 허용 매트릭스(KPI ⊇ HIGHLIGHT) 통과.
- `dataBinding.*`은 [6.2](#62-poc-목표-서버-db-영속화-item-35--dod-5) `DataSource` + 필드(백엔드 경유). 프론트에 endpoint 노출 없음.
- `rules[].op`은 전부 [7-A.5](#7-a5-rule-dsl-선언형-규칙-언어) 허용 연산.

### 7-A.8 검수 기준 (DoD-11 대응)
- [ ] `generate_widget_schema` 산출물이 **JSON(코드 아님)** 이고 공통 키(widgetId/widgetType/title/description/layout/stylePreset/dataBinding/config/schemaVersion/createdBy/version) 충족.
- [ ] `validate_widget_schema`(Spring Boot)가 **Type/Config/Style/Field/Rule 화이트리스트** 위반을 거부한다(미등록 Type·Type 불일치 Config·미허용 Style·타입 불일치 바인딩·미지원 Rule 전부 실패 처리).
- [ ] `update_widget_style`은 Type 유지·Style만 변경 / `change_widget_type`은 Config·Data Binding **재검증** 후 반영([5.4](#54-gemma-4-tool-call-화이트리스트--item-3)).
- [ ] `preview_widget`가 샘플 데이터로 렌더되고, 렌더 성공 시에만 Save 허용.
- [ ] `save_custom_widget`로 **`WidgetTemplate`** 저장 → `reuse_custom_widget`로 다른 페이지/프로젝트 재배치.
...
### 9.0 최초 위젯 생성 검수 기준 (v1.2 · DoD-11 선행)
> 전체 E2E 전에 **위젯 1개 생성·저장·복원**이 되는지 먼저 검증한다(구축 순서 [0.5.1](#051-poc-구축-순서-v12-신규) 5단계). **Gemma 4 없이 Mock JSON**으로도 통과해야 하며, 이후 Mock Provider를 Gemma 4로 교체한다.

**사용자 요청:** “전체 설비 상태 위젯을 추가해줘.”

**합격 기준**
- [ ] 구조화된 **`ADD_WIDGET` 결과 반환**(자유 코드 아님)
- [ ] 허용된 **10종 Widget Type**인지 검증
- [ ] **유형별 Config Schema** 검증([7-A.3.1](#7-a31-유형별-config-schema-10종))
- [ ] **Style Preset** 검증([7-A.3.2](#7-a32-style-preset-5-6종--v12))
- [ ] **Data Binding 필드** 검증(등록 소스·필드)
- [ ] **PostgreSQL에 `WidgetInstance` 저장**
- [ ] 프론트 화면에 위젯 **표시**
- [ ] 브라우저 **새로고침 후 동일 위젯 복원**
- [ ] **다른 세션에서도 서버 DB를 통해 동일 위젯 복원**

> 사전(POC 목표 구성): 2대 서버([0.5](#05-poc-인프라-구성--2대-물리-서버-v12-확정)) 기동 — Gemma 4 AI 서버 + Web·DB 서버, **외부 네트워크 차단(폐쇄망)**. 시작 전 데이터 초기화 + 브라우저 storage 정리(C6).
...
| **Versioning** | Widget Schema·WidgetTemplate의 버전 관리·롤백 |
