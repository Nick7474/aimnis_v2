...
| 액션 | 결과 |
|------|------|
| 헤더 `편집` 버튼 클릭 | Mode B 진입/복귀 (토글) |
| 딤 오버레이 클릭 (← 화살표) | Mode A 복귀 |
| 위젯 클릭 → FloatingToolbar 노출 → **설정** 버튼 클릭 | 우측 패널 열림 + 위젯 탭 포커스 |

> 위젯 클릭 자체, EditableSection 클릭, 드래그 드롭, 매핑 탭 이동 등은
> 패널 열림/닫힘에 관여하지 않음. FloatingToolbar의 설정 버튼이 유일한 위젯 진입점.
