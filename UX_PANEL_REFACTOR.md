...
```
┌────────────────────┬──────────────────┬────────────┐
│  Chat (280px)      │  Canvas (flex-1) │ Settings   │
│  [딤 오버레이]     │                  │ (슬라이드인)│
│  hover → ← 화살표  │                  │  320px     │
└────────────────────┴──────────────────┴────────────┘
```
- Chat 패널 크기 유지 (축소 없음)
- Chat 위에 `bg-black/60 backdrop-blur` 오버레이 씌움
- 오버레이 hover 시 흰색 ← 화살표 나타남
- 오버레이 클릭 → Mode A 복귀
...
- Chat translateX: `0 → -8px` (spring stiffness:300 damping:30)
- 딤 오버레이: opacity `0 → 1` (duration:0.2s)
- ← 화살표: group-hover opacity fade
