...
| 액션 | 결과 | 우선순위 |
|------|------|---------|
| 헤더 `편집` 버튼 클릭 | Mode B 진입/복귀 (토글) | P0 |
| 딤 오버레이 클릭 (← 화살표) | Mode A 복귀 | P0 |
| 오버레이 **위젯 클릭** (설정 의도) | 우측 패널 열림 + 위젯 탭 포커스 | P1 |
| **EditableSection** 클릭 (헤더·사이드바·맵 등) | 우측 패널 열림 + 해당 탭 포커스 | P1 |
| FloatingToolbar **데이터 연결** 버튼 | 우측 패널 열림 + 매핑 탭 | P2 |
| FloatingToolbar **설정** 버튼 (비위젯 섹션) | 우측 패널 열림 + 설정 탭 | P2 |
| 위젯 **드래그 → 우측 드롭존 드롭** | 우측 패널 열림 (드롭존 콘텐츠 확인) | P2 |

> 캔버스 빈 클릭, 매핑 탭 이동 등 수동 브라우징은 패널 열림/닫힘에 관여 안 함.
...
- Chat 컨테이너 `marginLeft`: `0 → -(chatPanelWidth - 70)px` (spring stiffness:300 damping:30)
  - chatPanelWidth에 관계없이 항상 70px 스트립이 화면에 남음
  - 예: 280px → -210px / 400px → -330px / 240px → -170px
